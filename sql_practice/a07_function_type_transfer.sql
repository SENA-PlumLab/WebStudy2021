...
# 형변환 유형
  ...
  2) 명시적 데이터 타입 변환
  	 ...
  	 1) to_char(문자열로): 숫자/날짜 ==> 문자열
  	 	ex) 숫자형/날짜형을 원하는 형식으로 화면에 출력할 때 사용
  	 2) to_number(숫자로): 문자열/날짜 ==> 숫자
  	 	ex) 문자형을 숫자로변환해서 where 조건문이나 select에서 다른 컬럼과 연산이 필요할 때,
  	 		입력/수정에서 문자열로 입력된 데이터를 숫자형으로 변환해서 등록할 때 사용됨.
  	 3) to_date(날짜로): 숫자/문자열 ==> 날짜
  	 	ex)입력되는 데이터가 문자열형이고, 특정한 형식으로 날짜형 타입의 컬럼에 입력할 때 사용됨.
 */
...
-- '문자열1'||'문자열2': 문자열을 이어줌
-- oracle에서는 +는 연산에서만 사용, ||은 문자열을 이어줄 때 사용하는 것으로 구분
-- '숫자형문자열'+'숫자형문자열': 자동형변환으로 연산처리 됨.
